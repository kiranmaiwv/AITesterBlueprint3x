"""Healthcare ETL QA Crew - Streamlit entry point.

This module is presentation only. Orchestration lives in
``hc_etl_qa_crew.services.pipeline`` and provider logic in
``hc_etl_qa_crew.data_gateway``, so the UI can be replaced without touching
either.
"""
...
from hc_etl_qa_crew.exceptions import ConfigurationError, RunInputError  # noqa: E402
from hc_etl_qa_crew.models import StageEvent  # noqa: E402
from hc_etl_qa_crew.services.pipeline import QAPipeline, new_run_id  # noqa: E402
from hc_etl_qa_crew.services.pipelines import parse_run_input  # noqa: E402
from hc_etl_qa_crew.ui import results as results_ui  # noqa: E402
from hc_etl_qa_crew.ui.components import (  # noqa: E402
    inject_theme,
    render_config_panel,
    render_header,
    render_input_area,
...
from hc_etl_qa_crew.ui.state import (  # noqa: E402
    clear_run,
    get_run,
    get_settings,
    init_state,
...
def main() -> None:
    st.set_page_config(
        page_title="Healthcare ETL QA Crew",
        page_icon="🏥",
        layout="wide",
    ...
    raw_pipelines = render_input_area(settings)

    try:
        parsed = parse_run_input(
            raw_pipelines or "",
            max_runs=settings.pipeline_max_runs,
    ...
    if not parsed.has_valid and (raw_pipelines or "").strip():
        st.error("No valid ETL pipeline names were found in the input.")
    if blocking:
        st.info("Configuration must be completed before a run can start. See the sidebar.")

    if start:
        _execute(settings, parsed)
...
def _execute(settings: Settings, parsed) -> None:
    """Run the pipeline synchronously while streaming genuine stage progress."""
    st.divider()
    st.subheader("Pipeline")

    total = len(parsed.valid)
    progress_bar = st.progress(0.0, text=f"Starting {total} pipeline(s)")
    ...
    def on_progress(pipeline_name: str, event: StageEvent) -> None:
        stage_state.setdefault(pipeline_name, {})[event.stage.value] = {
            "status": event.status.value,
            "message": event.message,
        }
        with stage_slots.get(pipeline_name, st.empty()).container():
            st.markdown(f"**{pipeline_name}**")
            render_stage_list(stage_state[pipeline_name])
        current_label.caption(f"{pipeline_name} — {event.stage.value}: {event.status.value}")
        finished = done["count"] + (1 if event.stage.value == "Artifacts" else 0)
        progress_bar.progress(
            min(1.0, (finished + 0.5) / max(total, 1)),
            text=f"{pipeline_name}: {event.stage.value}",
        )

    pipeline = QAPipeline(settings, progress=on_progress)
    run_id = new_run_id()

    with st.spinner("Running the ETL QA crew. This calls the LLM once per stage."):
        run = pipeline.run(
            parsed.valid,
