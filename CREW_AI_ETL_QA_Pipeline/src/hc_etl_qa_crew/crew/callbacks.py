...
class ProgressReporter:
    ...
    def _emit(self, pipeline_name: str, event: StageEvent) -> None:
        if self._callback is None:
            return
        try:
            self._callback(pipeline_name, event)
        except Exception:  # noqa: BLE001 - a broken UI sink must not kill a run
            logger.debug("progress callback raised", exc_info=True)

    def start(self, pipeline_name: str, event: StageEvent, message: str = "") -> StageEvent:
        event.status = StageStatus.RUNNING
        event.started_at = datetime.now()
        event.message = message
        self._emit(pipeline_name, event)
        return event
    ...
    def fail(self, pipeline_name: str, event: StageEvent, message: str) -> StageEvent:
        return self.finish(pipeline_name, event, StageStatus.FAILED, message)


def new_stage_set() -> list[StageEvent]:
    """The six stages every pipeline run goes through, in display order."""
